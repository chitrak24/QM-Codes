...
plt.plot(energies, hits, ls='solid', c='y')
plt.grid(True)

plt.axhline(ls='dotted',c='b')
plt.show()
...
def psi_normal(En):
    ...
    print("Normalisation Constant",N)
    return N * psi
...
for n in range(3):
    ...
    plt.subplot(3, 1, n + 1)
    plt.plot(x, psi_normal(En),c='k',ls='solid', lw=5)
    plt.grid(True)
    plt.xlabel("$x\longrightarrow $",fontsize=16)
    plt.ylabel("$\psi_{n}(x)\longrightarrow $",fontsize=16)
    plt.title("n={0}".format(n),size=5)
    plt.axhline(linestyle="--",c='r')
